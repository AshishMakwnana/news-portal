...
<div class="max-w-3xl mx-auto py-12">
    ...
    <form action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" value="{{ $news->title }}" class="mt-1 block w-full border rounded p-2" required>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <textarea name="body" rows="8" class="mt-1 block w-full border rounded p-2" required>{{ $news->body }}</textarea>
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium">Featured Image</label>
            <input type="file" name="featured_image" class="mt-1">
            @if($news->featured_image)
                <div class="mt-2"><img src="{{ asset('storage/' . $news->featured_image) }}" class="w-48 rounded"></div>
            @endif
        </div>
        <div class="mb-4">
