        ...
        <div class="space-y-6">
            @foreach ($news as $item)
                <article class="border p-4 rounded-lg">
                    ...
                    <p class="text-sm text-gray-500">{{ $item->published_at->format('M d, Y') }} • {{ $item->category?->name }}</p>
                    <p class="mt-2 text-gray-700">{{ Str::limit($item->excerpt, 150) }}</p>
                    <div class="mt-2">
                        @foreach($item->tags as $tag)
                            <a href="{{ route('tag.show', $tag->slug) }}" class="text-xs bg-gray-100 px-2 py-1 rounded">{{ $tag->name }}</a>
                        @endforeach
                    </div>
                </article>
            @endforeach
